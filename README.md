...
| **M3+** | pi 流式回复（`/v1/chat/stream`）+ 边收边念：pi 写一句，这边合成一句、播一句 | 完成 |

配套文档：`../三阶段实施细节与可复用代码.md`、`../pi-http-bridge-调用方法.md`、
`../TTS流式化-修改文档.md`、`../pi端配合-需求说明.md`、`../pi端配合-实现回复-2026-09-11.md`
...
前置：pi 已重启加载新 extension（`/v1/status` 里能看到 `"stream": true`；客户端有 10 分钟能力位缓存，
刚升级完想立刻生效就重进一次聊天页）。

- [ ] 唤醒问一个会答几句话的问题：**pi 还在写的时候就开始出声**（不是等整段答完）
- [ ] 回答落进聊天列表的时机是「pi 答完」，不是「播放结束」（边听边能在列表里看到）
- [ ] 多句回答按顺序念，**不串台词**、不重叠、不漏句
- [ ] 念到一半点「停止」→ 立刻静音，且**后面新到的句子不会再念出来**
- [ ] 停止后再问一轮，从头正常开始
- [ ] 把 pi 换成不支持流式的版本（或 `/v1/status` 里 `stream` 为 false）→ 自动退回整段模式，功能不受影响
- [ ] pi 在跑长工具（>10 秒没文本）时不出「连接中断」—— 心跳够用（离线用例已覆盖，真机再确认一次）
- [ ] 日志里那轮的判定是 `Ok(stream)`（Debug 页「最近一次请求」能看到）
...
com.pi.assistant/
  ...
  data/
    pi/                      pi bridge 客户端 + 错误状态机（M1 的灵魂）
      PiBridgeApi.kt         4 个端点：/v1/chat、/v1/chat/stream、/v1/status、/healthz
      PiStream.kt            SSE 帧读取与语义化（delta / done / error）
  ...
  voice/
    ...
    SpeechQueue.kt           边收边念：按句切分 + 单消费者播放队列
...
**12. 流式的失败判据是「起过没有」，不是 HTTP 码。** pi 的 SSE 响应头是在
「注入成功」那一刻才发的，所以 504/503 这类注入之后才发生的失败只能以 `error` 帧到达，
拿不到状态码。于是重试与否看 `error.started`：`false` 且这边一个 delta 都没收到，
才退回整段模式重来一次；`true`（已经吐过文本、可能已经念出声）绝不重试 ——
重试会让用户把开头听两遍。

**13. 边收边念靠一个单消费者队列，不是并发合成。** `SpeechQueue` 上游
（网络读）有多少收多少（`Channel.UNLIMITED`，永不阻塞读），下游一条一条念。
之所以不并发：两句同时开口会串台词。断句用句末标点，另有两条兜底 ——
攒够 48 字没标点就在最近的逗号处切一刀，流结束时把尾巴也念掉 ——
否则用户会遇到「静默 → 突然一整段」。
**落库时机**在「pi 答完」而不是「播放结束」：否则听完还要干等几秒才能在列表里看到回复。
...
- **只有语音回合走流式**：唤醒 / 按麦的回合用 `/v1/chat/stream` 边收边念；
  手打的消息仍走 `/v1/chat` 整段返回，聊天页不做逐字渲染
  （要的话得动 `ChatViewModel`/`ChatScreen`，加一层「正在输入」的临时状态）。
- **打断朗读 ≠ 中止 pi 的回合**：点停只静音、并丢弃后续句子，pi 那边照常把这一轮跑完
  （回复仍完整落库）。pi 的流式端点支持「断开即取消」，但那会把 TUI / 微信同一次运行
  一起停掉，所以客户端刻意没接。
- **`hint: "voice"` 是软约束**：让 pi 按口语稿回，但指令作用在模型上，偶尔仍会写长或带格式 ——
  客户端现有的 Markdown 清洗 + 切句继续当兜底。
- **流式播放期间没有音频文件**：裸 PCM16 存不成通用音频格式，所以这段音频用完即弃，
  不能像整段模式那样落盘成 wav 供导出。
- **流式被打断就丢弃**：中途点停 = 直接掐掉当前句，不会把已播的部分留档。
...
- **整段模式仍没有打断接口**：`/v1/chat` 是一问一答，用它的场景中途改口要等它答完。
- **厂商保活只能尽力而为**：代码侧只做到引导跳电池优化白名单，
  小米/华为/OPPO/vivo 的自启动与后台白名单仍需手动设置。
- **没跑过真机**：这份工程是在没有 JDK / Android SDK 的机器上写的 —— 代码经
  CI（release 构建 + R8，含 import 体检）验证可编译，但**运行时行为一概没上机验过**；
  真机上第一次跑若有问题，把日志贴出来即可。
