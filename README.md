...
- **快捷开关**：主界面标题栏的监听开关、长按桌面图标（开 / 关分开两条）、下拉快捷设置磁贴，都能开关后台监听
...
**4. 后台监听的四个入口**（同一件事，逻辑都走 `WakeControl`）
- 主界面顶栏 → 「pi 对话」右边的监听见开关（三态：关 / 监听中 / 未运行）
...
后台监听的四个入口（顶栏开关 / 设置页 / 桌面快捷方式 / 快捷设置磁贴）
...
- **后台监听的开关只有 `WakeControl` 一个出口**：顶栏开关、设置页、快捷方式、磁贴四处共用；新增入口请接它，别各写一套校验。
- **`WakeControl.State` 是开关状态的三态定义**：OFF / RUNNING / NOT_RUNNING。界面必须按三态显示 —— 配置开着但服务被杀时（NOT_RUNNING）要显式标出来，否则用户以为开着、喊半天没反应。
