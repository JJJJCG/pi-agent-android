...
private fun WakeSwitchChip(
    ...
    Surface(
        ...
        // 整块区域一个 clickable，Switch 设成纯展示（onCheckedChange = null），
        // 一次点击只会走一个入口。之前是外层 clickable + 内层 Switch 各自回调，
        // 点一下触发两次 onToggle，等于「关了又立刻开」，两个 set() 撞在一起。
        ...
        Row(
            ...
            Switch(
                checked = checked,
                // null = 不自己处理点击；点整块区域由上面的 clickable 接管
